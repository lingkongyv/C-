...
void SpeechManager::speechContest()
{
	...
	for (vector<int>::iterator it = v_src.begin(); it != v_src.end(); it++)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			double score = rand() % 401 + 600;  //600~1000
			score /= 10.0;
			//cout << score << endl;
			d.push_back(score);
		}
		...
		this->m_speaker[*it].score[this->index - 1] = ave;
		...
		if (num % 6 == 0)
		{
			...
			for (multimap<double, int, greater<double>>::iterator it = groupScore.begin(); it != groupScore.end(); it++)
			{
				cout << "编号：" << it->second
					...
					<< "  成绩：" << m_speaker[it->second].score[this->index - 1]
					<< endl;
			}
			int count = 0;
...
void SpeechManager::showScore()
{
	...
	for (vector<int>::iterator it = v.begin(); it != v.end(); it++)
	{
		cout << "选手编号：" << *it
			...
			<< "  得分：" << this->m_speaker[*it].score[this->index - 1]
			<< endl;
	}
	cout << endl;
	system("pause");
...
void SpeechManager::loadRecord()
{
	...
	while (ifs >> data)
	{
		...
		this->record.insert(make_pair(index1, v));
		index1++;
	}
	ifs.close();
}
